var store = [{
        ...
        "excerpt":"Jupyter Notebook을 환경에서 Python을 이용하여 Redis를 알아봅니다. redis-py를 이용하여 redis를 연결하고, 다음과 같이 데이터를 set/get을 수행하여 결과를 확인합니다. Connect import redis redisClient = redis.Redis(host='localhost', port=6379, decode_responses=True) redisClient.set('foo', 'bar') redisClient.get('foo') redisClient.set('mykey', 'Hello from Python!') value = redisClient.get('mykey') print(value) redisClient.zadd('vehicles', {'car' : 0}) redisClient.zadd('vehicles', {'bike' : 0}) vehicles = redisClient.zrange('vehicles', 0, -1)...","categories": ["cache"],
        "tags": ["redis","jupyter"],
        ...
        "excerpt":"Redis 란 무엇인가? Image from https://architecturenotes.co/redis/ Redis (“REmote DIctionary Service”) 오픈 소스이며 Key/Value 기반 데이터베이스 서버입니다. Redis에 대한 가장 정확한 설명은 데이터 구조 서버라고 합니다. Redis의 이러한 특정 특성으로 인해 개발자들 사이에서 인기를 얻고, 많은 solution으로 채택을 했습니다. 데이터가 이미 구조화되어 있고, 정렬되어있는 경우라면 ? 초기에는 memcahced 처럼 사용했었습니다. 그러나...","categories": ["cache"],
        "tags": ["redis","sentinel"],
